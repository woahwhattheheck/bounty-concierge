...
- `OWNED_FORK_PR`: a writable installed fork owned by the observed actor, an observed fork-parent binding to the declared upstream repository, branch/PR primitives, and an actual content-write path were observed. An actor-owned writable repository with an unobserved or mismatched parent fails closed to `HANDOFF_REQUIRED`.
...
With no installed fork, that observation yields `HANDOFF_REQUIRED` and the reason `UPSTREAM_CONNECTOR_ACCESS_UNKNOWN_NOT_PERMISSION_DENIAL`. A fork route becomes `OWNED_FORK_PR` only when `installed_fork_parent_repo` is freshly observed from repository metadata and matches `upstream_repo`; actor ownership and push access alone are not enough. Missing parent evidence yields `INSTALLED_FORK_PARENT_NOT_OBSERVED`, while a different parent yields `INSTALLED_FORK_PARENT_MISMATCH`.
