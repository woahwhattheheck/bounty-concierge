...
2. `concierge` package initialization seals the public reinvestment surface
   before normal callers can obtain either private bootstrap helper. On the
   first package import, pre-existing public/API/transport authority names in
   `sys.modules` are rejected rather than trusted.
...
Set these variables **before importing `concierge`** (package bootstrap imports
`concierge.reinvestment_allocator`) or before starting its CLI process:
...
modules. Its reviewed API/transport helpers construct one closure at package
bootstrap and retain the worker path, interpreter path, process primitives,
limits, protocol, and boot environment before callers receive the exported
functions.

Normal attempts to import `_reinvestment_allocator_api` or
`_reinvestment_allocator_transport` first still execute `concierge` package
initialization first, so the public closure is already sealed before those
mutable helper module objects are returned. The first package import also checks
`sys.modules` before any package side effects and fails closed if the public
reinvestment module, API helper, or transport helper was manually preseeded in
the import cache. A normal `importlib.reload(concierge)` after a completed seal
uses the package's completed-bootstrap sentinel rather than misclassifying the
already-loaded authority graph as a preseed attack.
...
interpreter, package files, exported functions, a completed package sentinel,
or the operating-system process boundary can fabricate local objects. Such an
object still cannot carry a valid signature from the externally held private
key. Consumers making consequential decisions must pin and verify the signer
...
transport-first imports, then attacks import-cache pre-seeding of the public
reinvestment module, private API helper, private transport helper, and all three
together. Each predecessor runs in a fresh child interpreter.
