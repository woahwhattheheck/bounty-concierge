...
The parent package is also ordinary-reload-safe for this reinvestment bootstrap. Python executes `importlib.reload(concierge)` in the existing package module dictionary. A private completion marker is written only after the initial child eviction + public seal succeeds. A later ordinary parent reload sees that retained completion state and does not evict or re-import the already-sealed reinvestment public/API/transport modules, so the exact-object reload guard cannot be stranded on obsolete child objects. After parent reload, ordinary imports still resolve to the same sealed child objects, `build_api` remains retired, and the captured compiler graph remains unchanged.
