...
| instruct | target text + speaker id + instruction text | yes | instruction tokens populate LLM `prompt_text`; frontend removes `llm_embedding` to avoid speaker-identity leakage |
...
- cross-lingual prompt-text removal is preserved; instruct tokens remain LLM `prompt_text` while instruct-mode `llm_embedding` stays removed;
