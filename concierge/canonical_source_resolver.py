...
def _identities_from_text(text: str) -> set[tuple[str, int, str]]:
    ...
    for match in _FULL_ISSUE_URL_RE.finditer(text):
        # Text extraction is only candidate discovery. Re-enter the same strict
        # structural validator used by listing_url/source_urls so regex capture
        # groups never become an alternate authority path.
        try:
            parsed = _https_url(match.group(0), "listing text issue URL")
        except CanonicalSourceInputError:
            continue
