...
def _draft(sponsor:str,work:dict[str,Any],route:dict[str,str],uid:str,amount:str,currency:str,evidence:str)->tuple[str,str]:
    ...
    body=(f"OWNER REVIEW DRAFT — DO NOT SEND UNTIL BOTH THE WORK REFERENCE AND PAYOUT ROUTE BELOW ARE INDEPENDENTLY VERIFIED.\n\nHi {sponsor},\n\nI’m preparing a request for payment of the {amount} {currency} reward offered for the sponsor claim unit below.\n\nCandidate work reference (caller-supplied; unverified): {work['canonical_url']}\nCandidate head (caller-supplied; unverified): {work['head_sha']}\nSponsor claim unit: {uid}\nReward-offer evidence: {evidence}\nCandidate payout route (caller-supplied; unverified): {route['type']} — {route['value']}\n\nBefore sending, independently verify that the candidate work reference is merged and mechanically belongs to this sponsor claim unit, and independently verify that the candidate payout route is owned or authorized for Bryce. Only after both checks pass should the offered payout be requested through that route.\n\nThanks,\nBryce")
...
def compile_adjudication_collection(payload:Any)->dict[str,Any]:
    ...
    if status=="reward_offered":
        ...
        if not usable:
            ...
            disposition="OWNER_REVIEW_REQUIRED"; reason="Sponsor-offered reward is host-authorized, but caller-supplied work identity and payout route are unverified; the work is not mechanically bound to the sponsor claim unit and the route is not independently bound to Bryce."
