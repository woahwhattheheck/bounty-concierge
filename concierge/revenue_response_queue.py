...
The authoritative compiler accepts a complete provider-read batch attested by a
credential-owning host. It does not accept a caller-provided fetch callback.
The attestation binds exact operator scope, host policy, authenticated provider
identity, and the reacquired provider snapshots before any action state can be
emitted.
"""
...
_SCHEMA = 'bounty-concierge.revenue-response-queue/v2'
...
_ID_RE = re.compile('^[A-Za-z0-9][A-Za-z0-9._:/+=@-]{0,255}$')
_KEY_RE = re.compile('^[a-z0-9][a-z0-9._:/-]{0,127}$')
_ENGAGEMENT_RE = re.compile('^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$')
...
_MANIFEST_KEYS = {'engagement_id', 'provider_thread_id', 'sent_message_id', 'buyer_route', 'authorized_reply_routes', 'offer_key', 'contact_policy', 'follow_up_after_hours'}
_POLICY_KEYS = {'schema', 'max_snapshot_age_seconds', 'auto_ack_grace_hours'}
_BATCH_KEYS = {'schema', 'key_id', 'provider', 'authenticated_principal', 'scope_sha256', 'policy_sha256', 'captured_at', 'snapshots', 'hmac_sha256'}
_RECEIPT_KEYS = {'schema', 'captured_at', 'scope_sha256', 'policy', 'policy_sha256', 'provider_authority', 'authority', 'summary', 'items', 'evidence_sha256', 'host_attestation_hmac_sha256'}
_SNAPSHOT_KEYS = {'thread_id', 'fetched_at', 'complete', 'messages'}
_MESSAGE_KEYS = {'id', 'kind', 'occurred_at', 'from_route', 'to_routes', 'related_message_id', 'sequence'}
_PRIORITY = {'HUMAN_REPLY': 0, 'HUMAN_REVIEW_REQUIRED': 1, 'ROUTE_REPAIR': 2, 'FOLLOW_UP_DUE': 3, 'WAIT_AUTO_ACK': 4, 'WAIT_BUYER_EVENT': 5, 'WAIT': 6}
...
def _exact_dict(value: Any, keys: set[str], context: str) -> dict[str, Any]:
    ...
    if set(value) != keys:
        ...
        raise RevenueResponseQueueError(f'{context} fields are not exact (missing={missing}, extra={extra})')
    return value
...
def _bounded_int(value: Any, *, field: str, minimum: int, maximum: int) -> int:
    ...
    if not minimum <= value <= maximum:
        raise RevenueResponseQueueError(f'{field} must be between {minimum} and {maximum}')
    return value
...
def _offer_key(value: Any) -> str:
    if type(value) is not str or value != value.strip() or (not _KEY_RE.fullmatch(value)):
        raise RevenueResponseQueueError('offer_key must be lowercase and use letters, digits, . _ : / -')
    return value
...
def _canonical_routes(value: Any, *, field: str) -> tuple[str, ...]:
    if type(value) is not list or not value or len(value) > 16:
        raise RevenueResponseQueueError(f'{field} must be a nonempty list with at most 16 routes')
...
def _validate_policy(policy: Any) -> dict[str, Any]:
    ...
    max_age = _bounded_int(obj['max_snapshot_age_seconds'], field='max_snapshot_age_seconds', minimum=30, maximum=3600)
    grace = _bounded_int(obj['auto_ack_grace_hours'], field='auto_ack_grace_hours', minimum=1, maximum=24 * 7)
    return {'schema': _POLICY_SCHEMA, 'max_snapshot_age_seconds': max_age, 'auto_ack_grace_hours': grace}
...
def _manifest_rows(manifest: Any) -> list[dict[str, Any]]:
    ...
    for index, raw in enumerate(manifest):
        ...
        routes = _canonical_routes(row['authorized_reply_routes'], field='authorized_reply_routes')
        ...
        normalized = {'engagement_id': engagement, 'provider_thread_id': thread_id, 'sent_message_id': sent_id, 'buyer_route': buyer, 'authorized_reply_routes': list(routes), 'offer_key': offer, 'contact_policy': contact_policy, 'follow_up_after_hours': follow_up}
...
def _scope_material(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [{key: value for key, value in row.items() if key != 'manifest_row_sha256'} for row in rows]

def _host_attestation_key() -> tuple[str, bytes]:
    key_id = os.environ.get(_ATTESTATION_KEY_ID_ENV)
    encoded = os.environ.get(_ATTESTATION_KEY_ENV)
    if type(key_id) is not str or not key_id or (not _KEY_RE.fullmatch(key_id)):
...
def _verified_batch(raw: Any, *, expected_scope_sha256: str, expected_policy_sha256: str) -> dict[str, Any]:
    batch = _exact_dict(raw, _BATCH_KEYS, 'provider_read_batch')
    ...
    return {'provider': provider, 'authenticated_principal': principal, 'captured_at': captured_at, 'snapshots': batch['snapshots'], 'key_id': key_id, '_attestation_key': key}
...
def _message(raw: Any, *, index: int, fetched_at: datetime) -> dict[str, Any]:
    ...
    if type(item['to_routes']) is not list or len(item['to_routes']) > 32:
        raise RevenueResponseQueueError('message.to_routes must be a list with at most 32 routes')
    ...
    return {'id': message_id, 'kind': kind, 'occurred_at': occurred_at, 'from_route': from_route, 'to_routes': to_routes, 'related_message_id': related, 'sequence': sequence}
...
def _snapshot(raw: Any, *, expected_thread_id: str, batch_captured_at: datetime, now: datetime, max_snapshot_age_seconds: int) -> dict[str, Any]:
    ...
    if type(raw_messages) is not list or len(raw_messages) > _MAX_MESSAGES_PER_THREAD:
        raise RevenueResponseQueueError(f'snapshot.messages must be a list with at most {_MAX_MESSAGES_PER_THREAD} messages')
    messages_by_id: dict[str, dict[str, Any]] = {}
    signatures: dict[str, str] = {}
    sequences: dict[int, str] = {}
    for index, raw_message in enumerate(raw_messages):
        message = _message(raw_message, index=index, fetched_at=fetched_at)
        semantic = {'id': message['id'], 'kind': message['kind'], 'occurred_at': _utc_text(message['occurred_at']), 'from_route': message['from_route'], 'to_routes': list(message['to_routes']), 'related_message_id': message['related_message_id'], 'sequence': message['sequence']}
        signature = _canonical_json(semantic)
    ...
    messages = sorted(messages_by_id.values(), key=lambda item: (item['occurred_at'], item['sequence'], item['id']))
    return {'thread_id': thread_id, 'fetched_at': fetched_at, 'messages': messages}

def _safe_snapshot_digest(snapshot: dict[str, Any]) -> str:
    rows = []
    for item in snapshot['messages']:
        rows.append({'id_sha256': _hash_text(item['id']), 'kind': item['kind'], 'occurred_at': _utc_text(item['occurred_at']), 'from_route_sha256': _hash_text(item['from_route']), 'to_route_sha256': sorted((_hash_text(route) for route in item['to_routes'])), 'related_message_id_sha256': _hash_text(item['related_message_id']) if item['related_message_id'] is not None else None, 'sequence': item['sequence']})
...
def _classify(row: dict[str, Any], snapshot: dict[str, Any], *, now: datetime, auto_ack_grace_hours: int) -> dict[str, Any]:
    ...
    anchor = [message for message in messages if message['id'] == row['sent_message_id']]
    if len(anchor) != 1:
    ...
    qualifying_outbounds = [message for message in messages if message['kind'] == 'outbound' and (message['occurred_at'], message['sequence'], message['id']) >= (anchor_message['occurred_at'], anchor_message['sequence'], anchor_message['id']) and authorized.intersection(message['to_routes'])]
    baseline = max(qualifying_outbounds, key=lambda item: (item['occurred_at'], item['sequence'], item['id']))
    baseline_id = baseline['id']
    baseline_order = (baseline['occurred_at'], baseline['sequence'], baseline_id)
    after_baseline = [message for message in messages if (message['occurred_at'], message['sequence'], message['id']) > baseline_order]
    exact_human = [message for message in after_baseline if message['kind'] == 'human_inbound' and message['from_route'] in authorized]
    unbound_human = [message for message in after_baseline if message['kind'] == 'human_inbound' and message['from_route'] not in authorized]
    bounces = [message for message in after_baseline if message['kind'] == 'bounce' and message['related_message_id'] == baseline_id]
    auto_acks = [message for message in after_baseline if message['kind'] == 'automated_inbound' and message['from_route'] in authorized]
    event: dict[str, Any] | None = None
    reasons: list[str] = []
    follow_up_due_at: datetime | None = None
    if exact_human:
        event = max(exact_human, key=lambda item: (item['occurred_at'], item['sequence'], item['id']))
        state = 'HUMAN_REPLY'
        reasons = ['AUTHORIZED_HUMAN_REPLY_AFTER_LATEST_OUTBOUND']
    elif unbound_human:
        event = max(unbound_human, key=lambda item: (item['occurred_at'], item['sequence'], item['id']))
        state = 'HUMAN_REVIEW_REQUIRED'
        reasons = ['HUMAN_REPLY_FROM_UNBOUND_ROUTE']
    elif bounces:
        event = max(bounces, key=lambda item: (item['occurred_at'], item['sequence'], item['id']))
        state = 'ROUTE_REPAIR'
        reasons = ['LATEST_OUTBOUND_BOUNCED']
    elif row['contact_policy'] == 'wait_for_buyer_event':
        if auto_acks:
            event = max(auto_acks, key=lambda item: (item['occurred_at'], item['sequence'], item['id']))
    ...
    latest_provider_event = max(messages, key=lambda item: (item['occurred_at'], item['sequence'], item['id']), default=baseline)
    route_hashes = sorted((_hash_text(route) for route in row['authorized_reply_routes']))
    return {'engagement_id': row['engagement_id'], 'offer_key': row['offer_key'], 'state': state, 'priority': _PRIORITY[state], 'reason_codes': reasons, 'contact_policy': row['contact_policy'], 'manifest_row_sha256': row['manifest_row_sha256'], 'buyer_route_sha256': _hash_text(row['buyer_route']), 'authorized_reply_routes_sha256': _sha_json(route_hashes), 'latest_outbound_at': _utc_text(baseline['occurred_at']), 'latest_provider_event_at': _utc_text(latest_provider_event['occurred_at']), 'follow_up_due_at': _utc_text(follow_up_due_at) if follow_up_due_at else None, 'thread_id_sha256': _hash_text(row['provider_thread_id']), 'anchor_message_id_sha256': _hash_text(row['sent_message_id']), 'latest_outbound_message_id_sha256': _hash_text(baseline_id), 'evidence_message_id_sha256': _hash_text(event['id']) if event else None, 'snapshot_evidence_sha256': _safe_snapshot_digest(snapshot)}

def compile_revenue_response_queue(manifest: Any, provider_read_batch: Any, *, policy: Any) -> dict[str, Any]:
    """Compile an authoritative queue from a host-attested provider-read batch.

    The public API deliberately accepts no fetch callback and no caller-selected
    current time. The credential-owning host must reacquire all exact threads,
    sign the complete batch with the host-only attestation key, and bind the
    canonical manifest/policy digests before this function is called.
    """
    rows = _manifest_rows(manifest)
    normalized_policy = _validate_policy(policy)
    scope_sha256 = _sha_json(_scope_material(rows))
    policy_sha256 = _sha_json(normalized_policy)
    ...
    raw_snapshots = batch['snapshots']
    if len(raw_snapshots) != len(rows):
    ...
    for index, raw in enumerate(raw_snapshots):
        ...
        thread_id = _compact_id(thread_id, field=f'snapshots[{index}].thread_id')
    ...
    expected_threads = {row['provider_thread_id'] for row in rows}
    if set(raw_by_thread) != expected_threads:
        raise RevenueResponseQueueError('provider_read_batch thread set does not exactly match manifest')
    items: list[dict[str, Any]] = []
    for row in rows:
        snapshot = _snapshot(raw_by_thread[row['provider_thread_id']], expected_thread_id=row['provider_thread_id'], batch_captured_at=batch['captured_at'], now=current, max_snapshot_age_seconds=normalized_policy['max_snapshot_age_seconds'])
    ...
    authority = {'provider_read_attested': True, 'scope_attested': True, 'policy_attested': True, 'receipt_host_attested': True, 'send_message': False, 'reply_to_buyer': False, 'mutate_provider': False, 'change_contact_policy': False, 'recognize_revenue': False, 'recognize_payment': False}
    provider_authority = {'provider_sha256': _hash_text(batch['provider']), 'authenticated_principal_sha256': _hash_text(batch['authenticated_principal']), 'key_id': batch['key_id'], 'batch_captured_at': _utc_text(batch['captured_at'])}
    semantic = {'schema': _SCHEMA, 'captured_at': _utc_text(current), 'scope_sha256': scope_sha256, 'policy': normalized_policy, 'policy_sha256': policy_sha256, 'provider_authority': provider_authority, 'authority': authority, 'summary': summary, 'items': items}
    signed = {**semantic, 'evidence_sha256': _sha_json(semantic)}
    receipt_hmac = hmac.new(batch['_attestation_key'], _canonical_json(signed).encode('utf-8'), hashlib.sha256).hexdigest()
    return {**signed, 'host_attestation_hmac_sha256': receipt_hmac}

def verify_revenue_response_queue_receipt(receipt: Any) -> bool:
    """Verify host authenticity, internal integrity, and current receipt freshness.

    This verifier deliberately accepts no caller-selected current time and no
    caller-selected key. It is meaningful only in a host that protects the
    attestation secret from untrusted request/input data.
    """
    try:
        ...
        if type(signature) is not str or _SHA_RE.fullmatch(signature) is None:
            return False
        provider_authority = obj['provider_authority']
        if type(provider_authority) is not dict:
            return False
        key_id, key = _host_attestation_key()
        if provider_authority.get('key_id') != key_id:
            return False
        signed = dict(obj)
        signed.pop('host_attestation_hmac_sha256')
        expected_hmac = hmac.new(key, _canonical_json(signed).encode('utf-8'), hashlib.sha256).hexdigest()
        if not hmac.compare_digest(signature, expected_hmac):
            return False
        evidence = signed.get('evidence_sha256')
        if type(evidence) is not str or _SHA_RE.fullmatch(evidence) is None:
            return False
        semantic = dict(signed)
        ...
        if captured > now + timedelta(seconds=_MAX_FUTURE_SKEW_SECONDS):
            return False
        if (now - captured).total_seconds() > normalized_policy['max_snapshot_age_seconds']:
            return False
        return True
