...
def verify_revenue_response_queue_receipt(receipt: Any) -> bool:
    try:
        ...
        if type(signature) is not str or not _SHA_RE.fullmatch(signature):
            return False
        key_id, key, authorized_provider, authorized_principal_sha = _host_attestation_context()
        pa = obj["provider_authority"]
        if type(pa) is not dict or pa.get("key_id") != key_id:
            return False
        if pa.get("provider_sha256") != _hash_text(authorized_provider):
            return False
        if pa.get("authenticated_principal_sha256") != authorized_principal_sha:
            return False
