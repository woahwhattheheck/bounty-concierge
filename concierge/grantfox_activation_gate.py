...
"""Fail-closed compositor for GrantFox queue, source, fulfillment, and lifecycle receipts."""
...
def compile_activation(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if qid[:3] != sid[:3] or qid[:3] != fid[:3] or qid[:3] != cid[:3]:
    ...
    dependency_source = _obj(
        fulfillment_dependency.get("source_receipt"),
        "fulfillment_receipt.dependency_receipt.source_receipt",
    )
    source_digest = _text(
    ...
    if (
        ...
        raise GrantFoxActivationInputError(
            "fulfillment receipt is anchored to a dependency receipt for a different source receipt"
        )
    ...
    ddisp = _text(
        fulfillment_dependency.get("dependency_disposition"),
        "fulfillment_receipt.dependency_receipt.dependency_disposition",
    )
    fdisp = _text(
    ...
    body = {
        ...
        "anchors": {
            ...
            "dependency_receipt_sha256": _text(
                fulfillment_dependency.get("dependency_receipt_sha256"),
                "fulfillment_receipt.dependency_receipt.dependency_receipt_sha256",
            ),
