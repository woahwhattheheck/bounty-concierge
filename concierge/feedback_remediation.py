"""Evidence-bound remediation packets for explicit paid-work feedback.

The packet digest is self-integrity only. Continuity authority comes from a
complete verified chain plus an independently retained expected prior head
Digest supplied by the caller/CLI.
"""
...
ADVANCE_SCHEMA = "bounty-feedback-remediation-advance/v2"
PACKET_SCHEMA = "bounty-feedback-remediation-packet/v2"
MAX_JSON_BYTES = 1_000_000
MAX_EVENTS = 500
MAX_OBLIGATIONS = 2_000
MAX_CHAIN_PACKETS = 1_000
...
AUTHORITY = {
    ...
    "chain_head_is_external_authority": True,
}
...
def strict_json_loads(text_value: str) -> Any:
    if not isinstance(text_value, str):
        fail("JSON input must be text")
    if len(text_value.encode("utf-8")) > MAX_JSON_BYTES:
...
def exact(value: Mapping[str, Any], required: Iterable[str], name: str) -> None:
    ...
    if got != want:
        fail("%s keys mismatch; missing=%s extra=%s" % (name, sorted(want - got), sorted(got - want)))
...
def norm_submission(value: Any) -> Dict[str, Any]:
    ...
    keys = {
        "submission_id", "canonical_source", "submitted_head_sha", "artifact_revision",
    ...
    return {
        ...
        "canonical_source": text(item["canonical_source"], "submission.canonical_source", MAX_SOURCE_CHARS, one_line=True),
        ...
        "artifact_evidence_digest": h64(item["artifact_evidence_digest"], "submission.artifact_evidence_digest"),
...
def obligation(source: str, event: Mapping[str, Any]) -> Dict[str, Any]:
    return {
        "obligation_id": obligation_id(source, event),
        "source_event_id": event["source_event_id"],
        "event_fingerprint": event_fp(event),
        ...
        "blocking": event["disposition"] != "NON_BLOCKING",
        "target_head_sha": event["target_head_sha"],
        "target_artifact_revision": event["target_artifact_revision"],
        "hints": copy.deepcopy(event["hints"]),
        "state": "OPEN",
        "resolution": None,
    }
...
def norm_events(values: Any, submission: Mapping[str, Any], existing: Optional[Mapping[str, str]] = None) -> List[Dict[str, Any]]:
    ...
    return sorted(
        by_id.values(),
        key=lambda e: (e["target_artifact_revision"], e["occurred_at"], e["source_event_id"], e["evidence_digest"]),
    )
...
def build_packet(
    ...
    ordered = sorted(
        (copy.deepcopy(x) for x in items),
...
def norm_resolution(value: Any, index: int, successor: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    item = obj(value, name)
    exact(item, {"obligation_id", "successor_artifact_evidence_digest", "note"}, name)
    evidence = h64(item["successor_artifact_evidence_digest"], name + ".successor_artifact_evidence_digest")
    if evidence != successor["artifact_evidence_digest"]:
        fail("%s evidence digest is not the successor artifact evidence digest" % name)
    note = item["note"]
    if note is not None:
        note = text(note, name + ".note", MAX_NOTE_CHARS)
    return {
        "obligation_id": safe_id(item["obligation_id"], name + ".obligation_id"),
        "successor_artifact_evidence_digest": evidence,
        "successor_head_sha": successor["submitted_head_sha"],
        "successor_artifact_revision": successor["artifact_revision"],
        "note": note,
    }
...
def norm_packet_obligation(value: Any, index: int, source: str) -> Dict[str, Any]:
    ...
    item = obj(value, name)
    keys = {
        "obligation_id", "source_event_id", "event_fingerprint", "authority", "kind", "occurred_at",
        "evidence_digest", "summary", "disposition", "blocking", "target_head_sha",
        "target_artifact_revision", "hints", "state", "resolution",
    }
    exact(item, keys, name)
    synthetic = {
        "submitted_head_sha": h40(item["target_head_sha"], name + ".target_head_sha"),
    ...
    if state == "OPEN":
        ...
        exact(
            data,
            {"successor_artifact_evidence_digest", "successor_head_sha", "successor_artifact_revision", "note"},
            name + ".resolution",
        )
        note = data["note"]
        if note is not None:
        ...
        normalized_resolution = {
            "successor_artifact_evidence_digest": h64(
                data["successor_artifact_evidence_digest"], name + ".resolution.successor_artifact_evidence_digest"
            ),
            "successor_head_sha": h40(data["successor_head_sha"], name + ".resolution.successor_head_sha"),
            "successor_artifact_revision": positive(
                data["successor_artifact_revision"], name + ".resolution.successor_artifact_revision"
            ),
            "note": note,
        }
...
def verify_packet(value: Any) -> Dict[str, Any]:
    ...
    keys = {
        "schema", "generation", "submission", "predecessor_packet_digest", "obligations", "counts",
    ...
    items = [
        norm_packet_obligation(x, i, submission["canonical_source"])
    ...
    for item in items:
        ...
        if resolution is not None:
            ...
            if resolution["successor_artifact_revision"] == submission["artifact_revision"]:
                if resolution["successor_head_sha"] != submission["submitted_head_sha"]:
                    fail("current-revision resolution head differs from current submission")
                if resolution["successor_artifact_evidence_digest"] != submission["artifact_evidence_digest"]:
                    fail("current-revision resolution evidence differs from current submission")
...
def verify_chain(values: Sequence[Any], expected_head_digest: Optional[str] = None) -> List[Dict[str, Any]]:
    raw = arr(list(values), "packet chain", MAX_CHAIN_PACKETS)
    if not raw:
    ...
    for index, packet in enumerate(verified):
        ...
        for oid, old in old_by_id.items():
            ...
            if resolution["successor_artifact_evidence_digest"] != current_submission["artifact_evidence_digest"]:
                fail("packet chain resolution is not bound to current artifact evidence")
    ...
    if expected_head_digest is not None:
        expected = h64(expected_head_digest, "expected_head_digest")
        if verified[-1]["packet_digest"] != expected:
            fail("packet chain head does not match independently retained expected digest")
    return verified


def advance_remediation(value: Any, expected_prior_head_digest: str) -> Dict[str, Any]:
    root = obj(value, "advance input")
    exact(root, {"schema", "prior_chain", "successor_submission", "addressed", "new_feedback"}, "advance input")
    if root["schema"] != ADVANCE_SCHEMA:
        fail("unsupported advance schema")
    chain = verify_chain(root["prior_chain"], expected_prior_head_digest)
    prior = chain[-1]
    ...
    candidate = build_packet(successor, items, prior["generation"] + 1, prior["packet_digest"])
    # Never return a transition that the production chain verifier would reject.
    verify_chain(list(chain) + [candidate], candidate["packet_digest"])
    return candidate
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Compile, advance, or verify evidence-bound bounty feedback remediation chains.")
    subs = parser.add_subparsers(dest="command", required=True)
    compile_parser = subs.add_parser("compile")
    compile_parser.add_argument("input")
    advance_parser = subs.add_parser("advance")
    advance_parser.add_argument("input")
    advance_parser.add_argument("--expected-prior-head-digest", required=True)
    verify_parser = subs.add_parser("verify")
    verify_parser.add_argument("chain")
    verify_parser.add_argument("--expected-head-digest", required=True)
    args = parser.parse_args(argv)
    try:
        if args.command == "compile":
            write_json(compile_remediation(load_json(args.input)))
        elif args.command == "advance":
            write_json(advance_remediation(load_json(args.input), args.expected_prior_head_digest))
        else:
            chain = verify_chain(load_json(args.chain), args.expected_head_digest)
            head = chain[-1]
            write_json({
                "valid": True,
