...
class ActivationGateTests(unittest.TestCase):
    ...
    def test_preassignment_fulfillment_hold_does_not_block_application(self):
        receipt = self.compile(request(
            queue_disposition="APPLY_ELIGIBLE",
            fulfillment_disposition="HOLD",
            continuity_state="DISCOVERED",
        ))
        self.assertEqual(receipt["disposition"], "APPLY_ELIGIBLE")
    ...
    def test_dependency_source_swap_fails_closed(self):
        payload = request()
        payload["fulfillment_receipt"]["dependency_receipt"]["source_receipt"]["source_receipt_sha256"] = "f" * 64
