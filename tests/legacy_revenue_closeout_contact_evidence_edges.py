...
class ContactEvidenceEdgeTests(unittest.TestCase):
    ...
    def test_near_future_send_evidence_cannot_suppress_collection(self):
        future = datetime.now(timezone.utc) + timedelta(minutes=2)
        evidence = receipt(sent_at=future.isoformat().replace("+00:00", "Z"))
        with self.assertRaisesRegex(RevenueCloseoutInputError, "cannot be in the future"):
            scan_paid_pr(
                item(settlement_followup_evidence=evidence),
                session=FakeSession(merged_routes()),
            )
